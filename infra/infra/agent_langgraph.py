...
def synchronize_pyproject_dependencies():
    ...
    custom_model_folder = str(
        os.path.join(str(agent_langgraph_application_path), "agentic_workflow")
    )
...
if session_secret_key := os.environ.get(SESSION_SECRET_KEY):
    ...
    session_secret_cred = pulumi_datarobot.ApiTokenCredential(
        agent_langgraph_asset_name + " Session Secret Key",
...
agent_langgraph_deployment_endpoint: pulumi.Output[str] = cast(
    pulumi.Output[str], "None"
)
if os.environ.get("AGENT_DEPLOY") != "0":
    ...
    agent_langgraph_deployment_completions_endpoint = agent_langgraph_agent_deployment.id.apply(
    ...
    pulumi.export(
        ...
        agent_langgraph_deployment_completions_endpoint,
    )
...
agent_langgraph_app_runtime_parameters = [
    ...
    pulumi_datarobot.ApplicationSourceRuntimeParameterValueArgs(
        key=agent_langgraph_application_name.upper() + "_ENDPOINT",
        type="string",
        value=agent_langgraph_deployment_endpoint,
    ),
]
