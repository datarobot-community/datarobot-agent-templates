...
def synchronize_pyproject_dependencies():
    ...
    uv_lock_path = os.path.join(str(agent_crewai_application_path), "uv.lock")
    custom_model_folder = str(
        os.path.join(str(agent_crewai_application_path), "agentic_workflow")
    )
...
if len(os.environ.get("DATAROBOT_DEFAULT_EXECUTION_ENVIRONMENT", "")) > 0:
    ...
    if os.path.exists(
        ...
        agent_crewai_execution_environment = pulumi_datarobot.ExecutionEnvironment(
            resource_name=agent_crewai_asset_name + " Execution Environment",
            name=agent_crewai_asset_name + " Execution Environment",
...
agent_crewai_playground = pulumi_datarobot.Playground(
    name=agent_crewai_asset_name + " Agentic Playground",
    resource_name=agent_crewai_asset_name + " Agentic Playground",
...
agent_crewai_blueprint = pulumi_datarobot.LlmBlueprint(
    name=agent_crewai_asset_name + " LLM Blueprint",
    resource_name=agent_crewai_asset_name + " LLM Blueprint",
...
agent_crewai_deployment_endpoint: pulumi.Output[str] = cast(pulumi.Output[str], "None")
if os.environ.get("AGENT_DEPLOY") != "0":
    agent_crewai_prediction_environment = pulumi_datarobot.PredictionEnvironment(
        resource_name=agent_crewai_asset_name + " Prediction Environment",
        name=agent_crewai_asset_name + " Prediction Environment",
        platform=dr.enums.PredictionEnvironmentPlatform.DATAROBOT_SERVERLESS,
        opts=pulumi.ResourceOptions(retain_on_delete=False),
    )

    agent_crewai_registered_model_args = RegisteredModelArgs(
        resource_name=agent_crewai_asset_name + " Registered Model",
        name=agent_crewai_asset_name + " Registered Model",
    )

    agent_crewai_deployment_args = DeploymentArgs(
        resource_name=agent_crewai_asset_name + " Deployment",
        label=agent_crewai_asset_name + " Deployment",
    ...
    agent_crewai_agent_deployment = CustomModelDeployment(
        resource_name=agent_crewai_asset_name + " Chat Deployment",
        use_case_ids=[use_case.id],
    ...
    agent_crewai_deployment_completions_endpoint = agent_crewai_agent_deployment.id.apply(
    ...
    pulumi.export(
        ...
        agent_crewai_deployment_completions_endpoint,
    )
...
agent_crewai_app_runtime_parameters = [
    ...
    pulumi_datarobot.ApplicationSourceRuntimeParameterValueArgs(
        key=agent_crewai_application_name.upper() + "_ENDPOINT",
        type="string",
        value=agent_crewai_deployment_endpoint,
    ),
]
