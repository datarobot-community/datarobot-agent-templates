...
def synchronize_pyproject_dependencies():
    ...
    custom_model_folder = str(
        os.path.join(str(agent_nat_application_path), "agentic_workflow")
    )
...
if session_secret_key := os.environ.get(SESSION_SECRET_KEY):
    ...
    session_secret_cred = pulumi_datarobot.ApiTokenCredential(
        agent_nat_asset_name + " Session Secret Key",
...
agent_nat_deployment_endpoint: pulumi.Output[str] = cast(pulumi.Output[str], "None")
if os.environ.get("AGENT_DEPLOY") != "0":
    ...
    agent_nat_deployment_completions_endpoint = agent_nat_agent_deployment.id.apply(
    ...
    pulumi.export(
        ...
        agent_nat_deployment_completions_endpoint,
    )
...
agent_nat_app_runtime_parameters = [
    ...
    pulumi_datarobot.ApplicationSourceRuntimeParameterValueArgs(
        key=agent_nat_application_name.upper() + "_ENDPOINT",
        type="string",
        value=agent_nat_deployment_endpoint,
    ),
]
