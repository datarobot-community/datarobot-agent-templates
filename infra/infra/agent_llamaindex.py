...
if len(os.environ.get("DATAROBOT_DEFAULT_EXECUTION_ENVIRONMENT", "")) > 0:
    ...
    if DEFAULT_EXECUTION_ENVIRONMENT in agent_llamaindex_execution_environment_id:
        pulumi.info(
        ...
        agent_llamaindex_execution_environment = (
            pulumi_datarobot.ExecutionEnvironment.get(
                ...
                resource_name=agent_llamaindex_asset_name + " Execution Environment",
            )
        )
    else:
        pulumi.info(
    ...
    if os.path.exists(
        ...
        agent_llamaindex_execution_environment = pulumi_datarobot.ExecutionEnvironment(
            resource_name=agent_llamaindex_asset_name + " Execution Environment",
            name=agent_llamaindex_asset_name + " Execution Environment",
...
session_secret_cred = pulumi_datarobot.ApiTokenCredential(
    "agent_llamaindex Session Secret Key",
...
agent_llamaindex_playground = pulumi_datarobot.Playground(
    name=agent_llamaindex_asset_name + " Agentic Playground",
    resource_name=agent_llamaindex_asset_name + " Agentic Playground",
...
agent_llamaindex_blueprint = pulumi_datarobot.LlmBlueprint(
    name=agent_llamaindex_asset_name + " LLM Blueprint",
    resource_name=agent_llamaindex_asset_name + " LLM Blueprint",
...
if os.environ.get("AGENT_DEPLOY") != "0":
    agent_llamaindex_prediction_environment = pulumi_datarobot.PredictionEnvironment(
        ...
        name=agent_llamaindex_asset_name + " Prediction Environment",
    ...
    agent_llamaindex_registered_model_args = RegisteredModelArgs(
        resource_name=agent_llamaindex_asset_name + " Registered Model",
        name=agent_llamaindex_asset_name + " Registered Model",
    )

    agent_llamaindex_deployment_args = DeploymentArgs(
        resource_name=agent_llamaindex_asset_name + " Deployment",
        label=agent_llamaindex_asset_name + " Deployment",
    ...
    agent_llamaindex_agent_deployment = CustomModelDeployment(
        resource_name=agent_llamaindex_asset_name + " Chat Deployment",
        use_case_ids=[use_case.id],
...
if os.environ.get("MCP_DEPLOYMENT_ID"):
    ...
    agent_llamaindex_app_runtime_parameters.append(
        pulumi_datarobot.ApplicationSourceRuntimeParameterValueArgs(
